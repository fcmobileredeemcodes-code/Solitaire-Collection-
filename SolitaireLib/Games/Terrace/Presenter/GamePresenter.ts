...
export class GamePresenter extends GamePresenterBase<IGame> {
    ...
    private updateSizes_() {
        // Terrace has 9 columns (the 9 tableau piles)
    ...
    private layoutPiles_() {
        ...
        const bottomY = middleY + this.sizeY + scaledMargin * 2;

        // Row 1 (Top): Stock (col 0), Waste (col 1), Reserve (col 3, fanned horizontally)
        {
            const pile = this.game_.stock;
            const pileView = this.getPileView_(pile);
            pileView.rect = new Rect(this.sizeX, this.sizeY, xPos(0), topY);
        }
        {
            const pile = this.game_.waste;
            const pileView = this.getPileView_(pile);
            pileView.rect = new Rect(this.sizeX, this.sizeY, xPos(1), topY);
        }
        {
            const pile = this.game_.reserve;
            const pileView = this.getPileView_(pile);
            pileView.rect = new Rect(this.sizeX, this.sizeY, xPos(3), topY);
            pileView.fanXUp = 0.5 * scale;
        }

        // Row 2 (Middle): Foundations 1-8 (columns 0.5 to 7.5)
        for (let i = 0; i < this.game_.foundations.length; ++i) {
            const pile = this.game_.foundations[i] ?? error();
            const pileView = this.getPileView_(pile);
            pileView.rect = new Rect(this.sizeX, this.sizeY, xPos(i + 0.5), middleY);
        }

        // Row 3 (Bottom): Tableaux 1-9 (columns 0 to 8)
        for (let i = 0; i < this.game_.tableaux.length; ++i) {
