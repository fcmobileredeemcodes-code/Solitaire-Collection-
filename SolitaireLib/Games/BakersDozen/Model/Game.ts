...
export class Game extends GameBase implements IGame {
    ...
    protected *restart_(rng: prand.RandomGenerator) {
        ...
        // Deal all 52 cards face up into 13 tableau columns of 4 cards each.
        // SPECIAL DEAL RULE: whenever a King is dealt into a column, instead of placing it on top in dealing order,
        // slip it to the BOTTOM of that column (index 0).
        for (let i = 0; i < 4; ++i) {
            for (let j = 0; j < 13; ++j) {
    ...
    private isTableauDrop_(card: Card, pile: Pile): boolean {
        ...
        if (topCard) {
            ...
            // Empty columns are NEVER refilled!
            return false;
        }
    }
