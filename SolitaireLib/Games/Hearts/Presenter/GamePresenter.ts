...
export class GamePresenter extends TrickTakingGamePresenterBase<Game> {
    ...
    constructor(game: Game, rootView: IView) {
        ...
        // Listen for pass confirmation button clicks in the modal body
    ...
    protected override relayoutAll_() {
        ...
        this.centerStatusPanel_.innerHTML = `
            ...
            ${(this.game_.isPassingPhase) ? `<div style="font-size: 1.3vh; color: #aaa; margin-top: 0.3rem;">Passing cards...</div>` : ""}
        `;
        ...
        if (this.game_.isPassingPhase) {
            ...
            this.showModal_(
                "Passing Phase",
                `
                <div style="font-size: 1.6vh; color: #fff; margin-bottom: 0.4rem;">Pass 3 cards to the <strong>${dirText}</strong></div>
                <div style="font-size: 1.3vh; color: #ccc; margin-bottom: 0.8rem;">Selected: ${selectedCount} / 3</div>
                <button id="confirmPassButton" ${confirmEnabled ? "" : "disabled"} class="${confirmEnabled ? 'confirm' : 'disabled'}">Confirm Pass</button>
                `
            );
        } else {
            this.hideModal_();
